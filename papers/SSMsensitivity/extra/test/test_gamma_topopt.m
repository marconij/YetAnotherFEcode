...
[J0, dJ] = evaluate(d0, to, sys, damping, maxOrder, ...
...
for i = 1:nEl
    ...
    for j = 1:nPert
        ...
        % Evaluate objective
        J = evaluate(d, to, sys, damping, maxOrder);
...
%% Function
function [J, dJ] = evaluate(d, to, sys, damping, maxOrder, varargin)
    % Parse inputs
    parser = inputParser;
    ...
    sys.set_damping_rayleigh(damping, ...
    ...
    % Objective
    gammaOrder = maxOrder;
    J = imag(ssm.R(gammaOrder).coeffs(1));

    % Compute sensitivity
    if computeSensitivity
        sens = SensitivityGamma(ssm, gammaOrder);
        dJdd = sens.adjoint_sensitivity_topopt;
