function obj = build_model(p, varargin)
...
% INPUTS:
% p: vector with the parameters of the model.
% symmetry: boolean to indicate if the model is symmetric (optional, default is false).
% OUTPUTS:
...
parser = inputParser;
parser.addOptional('symmetry', false, @islogical);
parser.parse(varargin{:});

% Check if the model is symmetric
if parser.Results.symmetry
    pSize = 10; % number of parameters for the symmetric model
else
    pSize = 15; % number of parameters for the non-symmetric model
end

% Check size of the input vector
if length(p)~=pSize
    error('p must be a vector of dimesion %d.', pSize);
end
...
if parser.Results.symmetry
    model.harmonics = [...
         p(6)  p(7)  p(8)  p(9)  p(10) % top-left beam
         p(6) -p(7)  p(8) -p(9)  p(10) % top-right beam
        -p(6) -p(7) -p(8) -p(9) -p(10) % bottom-left beam
        -p(6)  p(7) -p(8)  p(9) -p(10) % bottom-right beam
    ];
else
    model.harmonics = [...
        p(6)   p(7)  p(8)   p(9)  p(10) % top-left beam
        p(6)  -p(7)  p(8)  -p(9)  p(10) % top-right beam
        p(11)  p(12) p(13)  p(14) p(15) % bottom-left beam
        p(11) -p(12) p(13) -p(14) p(15) % bottom-right beam
    ];
end
