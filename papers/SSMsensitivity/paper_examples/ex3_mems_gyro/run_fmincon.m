function [mu, orderNew, orderHistory] = run_fmincon(mu0, A, b, Aeq, beq, xl, xu, args, options, varargin)
    ...
    function [fval, fgrad] = fun(mu)
        ...
        % Modal analysis
        nModes = 10;
        [V, D] = eigs(sys.K, sys.M, nModes, 'sm');
        ...
        % The spurious mode is the one with the lowest frequency besides
        % the drive and sense ones
        spuriousIndex = min(setdiff(1:nModes, [driveIndex, senseIndex]));

        % Spurious mode
        phiSpurious = V(:, spuriousIndex);
        phiSpurious = phiSpurious / sqrt(phiSpurious.' * sys.M * phiSpurious); % mass normalization
        omegaSpurious = sqrt(D(spuriousIndex, spuriousIndex));

        % Sensitivity of the sense frequency
        dOmegaSpurious = zeros(1, sys.ndv);
        for i = 1:sys.ndv
            dOmegaSpurious(i) = phiSpurious.' * (-omegaSpurious^2 * sys.pM(i).coeffs + sys.pK(i).coeffs) * phiSpurious / (2 * omegaSpurious);
        end

        % Objective function and constraints
        fval = -omegaSpurious;
        fgrad = -dOmegaSpurious;
    end
    ...
    function stop = output_fun(mu, optimValues, state, doPlot)
        ...
        driveIndex = modal_assurance_criterion(V, args.modeRefDrive);
        senseIndex = modal_assurance_criterion(V, args.modeRefSense);
        spuriousIndex = min(setdiff(1:size(V, 2), [driveIndex, senseIndex]));
        ...
        switch state
            ...
            case 'iter'
                ...
                fprintf('Iter %4d: fD = %6.2f kHz, fS = %6.2f kHz, fSp = %6.2f kHz, fT = %6.2f kHz, err = %6.2e\n', ...
                    iter, omegaDrive / (2*pi) / 1e3, omegaSense / (2*pi) / 1e3, omegaSpurious / (2*pi) / 1e3, OmegaT / (2*pi) / 1e3, err)
