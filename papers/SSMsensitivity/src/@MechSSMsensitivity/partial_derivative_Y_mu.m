function pYm_mu = partial_derivative_Y_mu(obj, mOrder, mIdx, pR_mu, pdw_mu)
    ...
    % Initialize
    pYm_mu = repmat(struct('coeffs', zeros(obj.ssm.sys.n, 1)), 1, obj.ssm.sys.ndv);
    ...
    for i = 1:coeffsVm.n
        ...
        for dv = 1:obj.ssm.sys.ndv
            ...
            pdwu_mu = pdw_mu(uOrder).pdv(dv).coeffs(:, uIdx);
            pRkj_mu = pR_mu(kOrder).pdv(dv).coeffs(j);

            % Compute the Y vector at the multi-index m
            pYm_mu(dv).coeffs = pYm_mu(dv).coeffs + u(j) * (pRkj_mu * dwu + Rkj * pdwu_mu);
        end
    end
end
