function pVm_mu = partial_derivative_V_mu(obj, mOrder, mIdx, pR_mu)
    ...
    % Initialize
    pVm_mu = repmat(struct('coeffs', zeros(obj.ssm.sys.n, 1)), 1, obj.ssm.sys.ndv);
    ...
    for i = 1:coeffsVm.n
        ...
        for dv = 1:obj.ssm.sys.ndv
            ...
            % Compute the V vector at the multi-index m
            pVm_mu(dv).coeffs = pVm_mu(dv).coeffs + pRkj_mu * u(j) * wu;
        end
    end
end
