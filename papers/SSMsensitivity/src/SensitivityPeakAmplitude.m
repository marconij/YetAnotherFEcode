classdef SensitivityPeakAmplitude < MechSSMsensitivity
    % A class to compute the adjoint sensitivity of the peak amplitude.
    % Methods:
    %   SensitivityPeakAmplitude: the constructor.
    ...
    methods
        function obj = SensitivityPeakAmplitude(ssm)
        ...
        function dJ = adjoint_sensitivity(obj, rho, a, epsilon, xk, x, outDof)
            % Compute the adjoint sensitivity of the peak amplitude.
            % Inputs:
        ...
        function rhoAdj = adjoint_equation_rho(obj, rho, a, xk, x, outDof)
            ...
            pJ_rho = obj.partial_derivative_x_rho(rho, xk, x, outDof);
            pg_rho = obj.partial_derivative_rho(rho, a);

            % Solve adjoint equation for the reduced amplitude
            rhoAdj = -pJ_rho / pg_rho;
        end

        function wAdj = adjoint_equation_w(obj, rho, a, xk, x, outDof, rhoAdj)
            ...
            pJ_w = obj.partial_derivative_x_w(rho, xk, x, outDof);
            ...
            for mOrder = obj.ssm.maxOrder:-1:2
                ...
                for mIdx = 1:nUniqueMIs
                    ...
                    % Partial derivatives with respect to the manifold coefficients
                    pJ_wm = pJ_w(mOrder).coeffs(:, mIdx);
                    [pCohom_wm, pg_wm] = obj.partial_derivative_w(mOrder, mIdx, rho, a, wAdj);
                    ...
                    % Solve the adjoint equation for the manifold coefficients
                    rhs = -pJ_wm - pCohom_wm.' - rhoAdj * pg_wm.';
        ...
        function [omegaAdj, phiAdj] = adjoint_equation_omega_phi(obj, rho, a, epsilon, xk, x, outDof, rhoAdj, wAdj)
            ...
            pJ_omega = 0;
            [pCohom_omega, pg_omega] = obj.partial_derivative_omega(rho, a, epsilon, wAdj);

            % Partial derivatives with respect to the mode shape
            pJ_phi = obj.partial_derivative_x_phi(rho, xk, x, outDof);
            [wAdj_pCohom_phi, pg_phi] = obj.partial_derivative_phi(rho, a, epsilon, wAdj);
        ...
        function pg_rho = partial_derivative_rho(obj, rho, a)
            % Compute the partial derivative with respect to  the reduced
            % amplitude.
            % Inputs:
            %   rho: the reduced amplitude.
            %   a: the coefficient a.
        ...
        function [pCohom_wm, pg_wm] = partial_derivative_w(obj, mOrder, mIdx, rho, a, wAdj)
            % Compute the partial derivative with respect to the manifold
            % coefficients.
            % Inputs:
        ...
        function [pCohom_omega, pg_omega] = partial_derivative_omega(obj, rho, a, epsilon, wAdj)
            % Compute the partial derivative with respect to the natural
            % frequency.
            % Inputs:
            %   rho: the reduced amplitude.
            %   a: the coefficient a.
        ...
        function [wAdj_pCohom_phi, pg_phi] = partial_derivative_phi(obj, rho, a, epsilon, wAdj)
            % Comptue the partial derivative with respect to the mode
            % shape.
            % Inputs:
            %   rho: the reduced amplitude.
            %   a: the coefficient a.
        ...
        function [pCohom_mu, pg_mu] = partial_derivative_mu(obj, rho, a, wAdj)
            % Compute the partial derivative with respect to the design
            % variables.
            % Inputs:
            %   rho: the reduced amplitude.
            %   a: the coefficient a.
